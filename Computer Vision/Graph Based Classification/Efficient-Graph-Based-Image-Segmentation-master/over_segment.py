...
def create_segment_image(union_find, im):
    """Here we generate blank matrix and store the data of segmented matrix in it"""
    uf = union_find
...
if __name__ == '__main__':
    image = Image.open('./number plate.jpg')
    """We pass the image through image filter and median filter"""
    image.thumbnail((1200,600))
    imshow(create_segment_image(segment(image, 100), image), cmap='Accent') 
    """here we are calling Create segment function, segment function"""
    show()
    """This is the main loop for the program, where multiple function are being called"""